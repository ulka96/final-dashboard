...
const fetchBlogs = async () =>{
  ...
  const handleBlogEdit = async (blogId, updatedData) => {
    try {
  ...
  <Link to="/addBlog" >
    ...
    before:absolute before:left-0 before:right-0 before:top-0 before:h-0 before:w-full
     ...
     hover:after:h-2/4"> <span className="relative z-10">Add Blog</span></button>
  </Link>
      ...
      <thead className="bg-gray-200">
        ...
        <th className="py-2 px-4 border text-[14px] border-gray-300">ID</th>
          <th className="py-2 px-4 border text-[14px] border-gray-300">title</th>
           <th className="py-2 px-4 border text-[14px] border-gray-300">content</th>
           <th className="py-2 px-4 border text-[14px] border-gray-300">image url</th>
           <th className="py-2 px-4 border text-[14px] border-gray-300">video url</th>
        </tr>
      </thead>
      <tbody>
          {blogs && blogs.map((blog) => (
            <tr key={blog._id} className="odd:bg-white even:bg-gray-100">
              <td className="py-2 px-8 border border-gray-300 text-[14px]">{blog._id}</td>

              <td className="py-2 px-8 border text-[14px]  border-gray-300">{blog.title}</td>
              <td className="py-2 px-8 border text-[14px]  border-gray-300">{blog.content}</td>
              <td className="py-2 px-8 border text-[14px]  border-gray-300">{blog.image}</td>
              <td className="py-2 px-8 border text-[14px]  border-gray-300">{blog.videoUrl}</td>

                  <td className=" border text-[14px] items-baseline flex flex-col py-2 px-8 space-x-3  border-gray-300">
                      
              <Link to={`/blogs/${blog._id}`}>
                          <button onClick={() => handleBlogEdit(blog._id)} className="group relative min-h-[50px] w-40 
               ...
               before:duration-500 after:absolute after:right-1/4 after:top-0 after:z-0 after:h-0 after:w-1/4
                ...
                <button onClick={() => handleBlogDelete(blog._id)} className="text-red text-[14px] 
