...
const fetchCategories = async () =>{
  ...
  const handleDelete = async (categoryId) => {
    ...
    console.log("mongoid", category._id)
  };
  ...
  <Link to="/addCategory" >
    ...
    before:absolute before:left-0 before:right-0 before:top-0 before:h-0 before:w-full
     ...
     hover:after:h-2/4"> <span className="relative z-10">Add Category</span></button>
  </Link>
      ...
      <tbody>
          {categories && categories.map((category) => (
            <tr key={category._id} className="odd:bg-white even:bg-gray-100">
              <td className="py-2 px-8 border border-gray-300 text-[14px]">{category._id}</td>

              <td className="py-2 px-8 w-[150px] h-[150px] border border-gray-300">
                <img src={`http://localhost:3000/${category.categoryPic}`} className='w-full'/>
                
                </td>
              <td className="py-2 px-8 border text-[14px]  border-gray-300">{category.title}</td>

                  <td className=" border text-[14px] items-baseline flex flex-col py-2 px-8 space-x-3  border-gray-300">
                      
              <Link to={`/categories/${category._id}`}>
                          <button onClick={() => handleEdit(category._id)} className="group relative min-h-[50px] w-40 
               ...
               before:duration-500 after:absolute after:right-1/4 after:top-0 after:z-0 after:h-0 after:w-1/4
                ...
                <button onClick={() => handleDelete(category._id)} className="text-red text-[14px] 
